...
export default function SearchAddProduct() {
  ...
  useEffect(() => {
    api.getCategories().then(setCategories).catch((err) =>
      setLoadError(`Could not load categories: ${err.message}. Is the backend running at http://127.0.0.1:8000?`)
    );
    api.getRetailers().then(setRetailers).catch((err) =>
      setLoadError(`Could not load retailers: ${err.message}. Is the backend running at http://127.0.0.1:8000?`)
    );
  }, []);
  ...
  return (
    <div className="app">
      ...
      <div className="notice">
        This build implements <strong>P1-P5</strong>: catalog search/add, retailer ingestion
        (mock data), true price &amp; deal score, category-specific review signals, and a
        watchlist. The dashboard/comparison view (P6) is the remaining increment — see README.
      </div>

      {loadError && <div className="error" style={{ marginBottom: 20 }}>{loadError}</div>}

      {selectedProductId && (
        <ListingDetail productId={selectedProductId} onClose={() => setSelectedProductId(null)} />
      )}
            ...
            {results.map((r) => (
              <div className="result-item" key={r.product_id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button onClick={() => setSelectedProductId(r.product_id)} style={{ marginTop: 0 }}>
                    View details
                  </button>
                </div>
              </div>
