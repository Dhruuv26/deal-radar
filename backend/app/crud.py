...
class DuplicateWatchlistItemError(Exception):
    """Raised when a listing is already on the watchlist."""
...
def add_to_watchlist(db: Session, listing_id: int, threshold_score: float) -> models.WatchlistItem:
    """REQ-P5-1, REQ-P5-2."""
    if get_listing(db, listing_id) is None:
        raise ValueError(f"Listing id {listing_id} does not exist.")

    existing = (
        db.query(models.WatchlistItem)
        .filter(models.WatchlistItem.listing_id == listing_id)
        .first()
    )
    if existing:
        raise DuplicateWatchlistItemError(f"Listing {listing_id} is already on the watchlist.")

    item = models.WatchlistItem(listing_id=listing_id, threshold_score=threshold_score)
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def remove_from_watchlist(db: Session, watchlist_id: int) -> bool:
    """REQ-P5-1."""
    item = db.query(models.WatchlistItem).filter(models.WatchlistItem.watchlist_id == watchlist_id).first()
    if item is None:
        return False
    db.delete(item)
    db.commit()
